...
for i in range(len(df['Donchian_Upper'])):
    if (df['4'].iloc[i] > df['Donchian_Upper'].iloc[i] and df['EMA_50'].iloc[i] > df['EMA_200'].iloc[i]) and position == 0:
        ...
        trades.append(1 + ((df['4'].iloc[i] - entry_price) / entry_price))
pnl = (1 + ((balance - 100) / 100)) * 100

# basic backtest results
print('pnl:', pnl)
print('number of trades:', len(trades))
print('time period:', (len(df['Donchian_Upper']) * 15) / 60 / 24, 'days')
...
cum_returns = np.cumprod(np.array(trades))
