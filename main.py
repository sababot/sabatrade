...
console.print("[purple][bold]st[/bold] [white]initializing tensorflow")
...
ohlcv = fetch_data('ETH/USDT', '15m', 10)

console.print("[purple][bold]st[/bold] [white]training dataset with model")
...
class KNN(tf.keras.Model):
    ...
    def predict(self, X_test, show):
        predictions = []
        if show == True:
            with Progress() as progress:
                # Add a progress task
                task = progress.add_task("  Progress:", total=len(X_test))
...
predictions = knn.predict(tf.constant(X_test), True)


# Evaluate accuracy
accuracy = np.mean(np.array(predictions) == y_test)
console.print(f"[purple][bold]st[/bold] [white]Model Accuracy: {accuracy * 100:.2f}%")
#print(f"[st] Model Accuracy: {accuracy * 100:.2f}%")
...
for i in range(len(predictions)):
    ...
    if signal == 1:  # Buy signal
        ...
        if (position == 1 and (current_price - entry_price) / entry_price < -0.0025):
            ...
            console.print(f"[purple][bold]st[/bold] [white]Liquidation -1.00%")
